...
function main() {
  ...
  const outPath = path.join(docDir, "tasks.md");
  ...
  if (!fs.existsSync(designPath)) {
    console.error(`[ERROR] design.md not found: ${path.relative(process.cwd(), designPath)}`);
    console.error("Please run design_md for the same module first.");
    process.exit(2);
  }
  ...
  const mustHave = ["# 实现计划", "## 概述", "## 任务", "## 注意事项"];
